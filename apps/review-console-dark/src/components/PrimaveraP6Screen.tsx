...
export const PrimaveraP6Screen: React.FC = () => {
  ...
  const wbsSummary = [
    { level: "L1", labelHi: "????????? ??????", labelEn: "Project Master", pct: "100%", finished: isHi ? "1 / 1 ??????" : "1 / 1 Finished", active: isHi ? "0 ??????" : "0 Active", barWidth: "100%", statusColor: "bg-emerald-600 text-white font-extrabold" },
    { level: "L2", labelHi: "????? ????", labelEn: "Unit Level", pct: "95%", finished: isHi ? "19 / 20 ??????" : "19 / 20 Finished", active: isHi ? "1 ??????" : "1 Active", barWidth: "95%", statusColor: "bg-emerald-600 text-white font-extrabold" },
    { level: "L3", labelHi: "?????? ?????", labelEn: "System Package", pct: "72%", finished: isHi ? "45 / 62 ??????" : "45 / 62 Finished", active: isHi ? "17 ??????" : "17 Active", barWidth: "72%", statusColor: "bg-amber-500 text-slate-950 font-black" },
    { level: "L4", labelHi: "??-?????? / ???????", labelEn: "Sub-System / Module", pct: "50%", finished: isHi ? "120 / 240 ??????" : "120 / 240 Finished", active: isHi ? "80 ??????" : "80 Active", barWidth: "50%", statusColor: "bg-amber-500 text-slate-950 font-black" },
    { level: "L5", labelHi: "???? ?????", labelEn: "Work Package", pct: "15%", finished: isHi ? "25 / 165 ??????" : "25 / 165 Finished", active: isHi ? "140 ??????" : "140 Active", barWidth: "15%", statusColor: "bg-slate-200 text-slate-800 dark:bg-slate-800 dark:text-slate-200 font-bold" },
    { level: "L6", labelHi: "?????? ????? ???", labelEn: "Field Task Node", pct: "2%", finished: isHi ? "4 / 200 ??????" : "4 / 200 Finished", active: isHi ? "196 ??????" : "196 Active", barWidth: "2%", statusColor: "bg-slate-200 text-slate-800 dark:bg-slate-800 dark:text-slate-200 font-bold" },
  ];

  // Master schedule activities with high contrast light/dark mode styling
  const masterActivities = [
    { id: "HCU3-000", level: "L1", nameHi: "????????????? ??????? ????? 3 ???????? ????", nameEn: "Hydrocracker Expansion Unit 3 Project Initiation", disciplineHi: "???????", disciplineEn: "Management", discColor: "bg-indigo-100 text-indigo-800 border-indigo-300 dark:bg-indigo-950 dark:text-indigo-300 dark:border-indigo-800", status: "Completed", start: "2024-01-01", finish: "2024-03-31", progress: 100 },
    { id: "HCU3-ENG-105", level: "L3", nameHi: "????? ??????? ??? ?? ??? ??????? ??????????? ??????", nameEn: "Detailed Engineering Design for Main Reactor Vessel", disciplineHi: "???????????", disciplineEn: "Engineering", discColor: "bg-purple-100 text-purple-800 border-purple-300 dark:bg-purple-950 dark:text-purple-300 dark:border-purple-800", status: "In Progress", start: "2024-04-01", finish: "2024-11-15", progress: 75 },
    { id: "HCU3-CIV-202", level: "L4", nameHi: "???? ?? ????? ?? ??????? ????? - ??? B", nameEn: "Foundation Excavation and Piling Works - Zone B", disciplineHi: "?????", disciplineEn: "Civil", discColor: "bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-950 dark:text-blue-300 dark:border-blue-800", status: "Delayed", start: "2024-09-01", finish: "2024-12-30", progress: 40 },
    { id: "HCU3-MEC-310", level: "L5", nameHi: "???? ???? ??? ??????????? (E-101 A/B) ?? ???????", nameEn: "Installation of High-Pressure Heat Exchangers (E-101 A/B)", disciplineHi: "????????", disciplineEn: "Mechanical", discColor: "bg-amber-100 text-amber-900 border-amber-300 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800", status: "Not Started", start: "2025-02-15", finish: "2025-05-20", progress: 0 },
    { id: "HCU3-INS-405-A", level: "L6", nameHi: "???? ???????????? ?? ??? ?????? ?? ??????????", nameEn: "Loop Checking and Calibration of Flow Transmitters (FT-201 to FT-250)", disciplineHi: "???????????????", disciplineEn: "Instrumentation", discColor: "bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-950 dark:text-emerald-300 dark:border-emerald-800", status: "Not Started", start: "2025-08-01", finish: "2025-09-15", progress: 0 },
  ];
  ...
  return (
    <main className={`lg:ml-[240px] ml-0 mt-[56px] flex-1 overflow-y-auto p-4 lg:p-6 min-h-[calc(100vh-56px)] flex flex-col gap-6 transition-colors ${
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className={`border rounded-lg px-4 py-2 text-xs font-black flex items-center gap-2 transition-all cursor-pointer shadow-xs ${
              isDarkMode ? 'bg-slate-900 border-slate-800 hover:bg-slate-800 text-slate-200' : 'bg-white border-slate-300 hover:bg-slate-100 text-slate-800'
            }`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div className={`border rounded-xl p-4 flex flex-col gap-1 shadow-xs transition-colors ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
        }`}>
          <span className="text-[10.5px] font-black tracking-wider uppercase text-slate-500 dark:text-slate-400">
            {isHi ? "??? ?????? WBS ?????" : "TOTAL MASTER WBS NODES"}
          </span>
          <span className="text-3xl font-black text-blue-700 dark:text-sky-400">508</span>
        </div>
      ...
      <div className="flex flex-col gap-3">
        <h3 className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2">
          ...
          {isHi ? "WBS ??????? ?????? ????????? (L1 - L6)" : "WBS Level-Wise Progress Summary"}
        </h3>
            ...
            <div 
              ...
              className={`border rounded-xl flex flex-col overflow-hidden transition-all cursor-pointer ${
                selectedLevel === item.level 
                  ? 'ring-2 ring-blue-600 border-blue-600 bg-blue-50/70 dark:bg-blue-900/30' 
                  : isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-slate-700' : 'bg-white border-slate-200 shadow-xs hover:border-blue-400'
              }`}
            >
              <div className="p-3.5 flex-1 flex flex-col justify-between gap-2">
                <div className="flex justify-between items-start">
                  <span className="font-black text-lg text-blue-700 dark:text-sky-400">{item.level}</span>
      ...
      <div className={`border rounded-xl flex flex-col shadow-xs overflow-hidden ${
        ...
        <div className={`p-4 border-b flex flex-wrap items-center gap-4 ${
          ...
          <div className="ml-auto relative w-full sm:w-64">
            ...
            <input
              ...
              placeholder={isHi ? "??????? ID ?????..." : "Search Activity ID..."}
        ...
        {/* Table - HIGH CONTRAST IN LIGHT MODE AND DARK MODE */}
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse min-w-[900px]">
            <thead className={`border-b ${
              isDarkMode ? 'bg-slate-950/90 border-slate-800 text-slate-300' : 'bg-slate-200/70 border-slate-300 text-slate-900'
            }`}>
              <tr>
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800' : 'divide-slate-200'}`}>
              {filteredActivities.map((row) => (
                <tr key={row.id} className={`transition-colors ${isDarkMode ? 'hover:bg-slate-800/60' : 'hover:bg-blue-50/60'}`}>
                  ...
                  {/* P6 Activity ID: Deep Black in Light Mode, White in Dark Mode */}
                  <td className="py-3.5 px-4 font-mono font-black text-slate-900 dark:text-slate-100 whitespace-nowrap">
                    {row.id}
                  </td>
                  {/* Master Activity Name: Deep Black in Light Mode, White in Dark Mode */}
                  <td className="py-3.5 px-4 font-black text-slate-900 dark:text-white whitespace-normal">
                  ...
                  <td className="py-3.5 px-4">
                    {row.status === 'Completed' ? (
                      <span className="flex items-center gap-1.5 text-emerald-700 dark:text-emerald-400 text-xs font-black">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600" /> {isHi ? "?????" : "Completed"}
                      </span>
                    ) : row.status === 'Delayed' ? (
                      <span className="flex items-center gap-1.5 text-red-600 dark:text-red-400 text-xs font-black">
                        <AlertTriangle className="w-4 h-4 text-red-600" /> {isHi ? "???????" : "Delayed"}
                      </span>
                    ) : row.status === 'In Progress' ? (
                      <span className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400 text-xs font-black">
                        <Clock className="w-4 h-4 text-amber-500" /> {isHi ? "?????? ??" : "In Progress"}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-xs font-bold">
                        <PauseCircle className="w-4 h-4 text-slate-400" /> {isHi ? "???? ????" : "Not Started"}
                      </span>
                    )}
                  </td>
                  {/* Planned Dates: Deep Slate in Light Mode, Soft Slate in Dark Mode */}
                  <td className="py-3.5 px-4 font-mono text-[11px] font-bold text-slate-800 dark:text-slate-300 whitespace-nowrap">
                  ...
                  {/* Progress %: Deep Black in Light Mode */}
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-mono text-xs font-black min-w-[36px] ${
                        row.status === 'Delayed' ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-slate-100'
                      }`}>
                        {row.progress}%
                      </span>
                    </div>
        ...
        {/* Footer / Pagination */}
        <div className={`p-3.5 border-t flex justify-between items-center text-xs text-slate-600 dark:text-slate-400 font-bold ${
          isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-100/80 border-slate-200'
        }`}>
          <span>{isHi ? "508 ????? ??? ?? 1-5 ?????????" : "Showing 1-5 of 508 nodes"}</span>
          <div className="flex gap-1">
            <button className="px-2.5 py-1 border border-slate-300 dark:border-slate-700 rounded bg-white dark:bg-slate-900 disabled:opacity-50 text-xs font-bold cursor-not-allowed" disabled>&lt; {isHi ? "?????" : "Prev"}</button>
            ...
            <button className="px-2.5 py-1 border border-slate-300 dark:border-slate-700 rounded bg-white dark:bg-slate-900 text-xs font-bold">{isHi ? "????" : "Next"} &gt;</button>
          </div>
        </div>
